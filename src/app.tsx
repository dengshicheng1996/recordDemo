...
function rrweb() {
  record({
    emit(event) {
      console.log(event)
      window.chunks = [...(window.chunks || []), event];
    },
  });
}
window.chunklist = [];
function h2c() {
  setInterval(() => {
    html2canvas(document.body, {
      ...
      // console.log(window.chunklist)
      window.chunklist.push(base64);
    });
  }, 1000);
}
window.videos = [];
  ...
  navigator.mediaDevices
    ...
    .then((stream) => {
      ...
      recorder.ondataavailable = async (event: any) => {
        console.log(event)
        window.videos = [...(window.videos || []), event.data];
      };
    });
}
...
function timecat () {
  ...
  recorder.onData(async (data, next) => {
